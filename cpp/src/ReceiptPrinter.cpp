...
ReceiptPrinter::ReceiptPrinter() : ReceiptPrinter(NumberOfReceiptPrinters)
{
}
...
std::string ReceiptPrinter::presentQuantity(const ReceiptItem &item)
{
	return ProductUnit::Each == item.getProduct().getUnit()
			? getFormattedNumberAsString(item.getQuantity(), MinimumPrecision)
			: getFormattedNumberAsString(item.getQuantity(), MaximumPrecision);
}
